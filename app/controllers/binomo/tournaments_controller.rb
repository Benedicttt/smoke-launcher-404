class Binomo::TournamentsController < ApplicationController
  ...
  def create
    ...
    File.open("./features/tournaments.feature", 'w+'){|f| f.puts("Feature: SMoke test generate #{Time.now}\n Scenario: Run step\n Given Pid process\n  ")}
    File.open("./features/tournaments.feature", 'a'){|f| f.puts(scenarios.join("\n"))}
    p params["generator"]
  end
