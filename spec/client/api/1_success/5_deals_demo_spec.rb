...
RSpec.describe "Deals success turbo" do
  ...
  context "params" do
    ...
    it { expect(@deals_demo[0]['data']["status"]).to eq "open" }
    ...
    it { expect(@deals_demo[1]["data"]['deals'][0]['entrie_rate']).to be_a(String)  }
    it { expect(@deals_demo[1]["data"]['deals'][0]['bet']).to eq 1000  }
    it { expect(@deals_demo[1]["data"]['deals'][0]['payment']).to eq "#{1000 + Assets.new.get('web', 'ru')[1][0]*10}.0" }
    ...
    it { expect(@deals_demo[1]["data"]['deals'][0]['end_rate']).to eq nil }
...
RSpec.describe "Deals success turbo" do
  ...
  context "params" do
    ...
    it { expect(@deals_demo[1]["data"]['deals'][0]['entrie_rate']).to be_a(String)  }
    it { expect(@deals_demo[1]["data"]['deals'][0]['bet']).to eq 1000  }
    it { expect(@deals_demo[1]["data"]['deals'][0]['payment']).to eq "#{1000 + Assets.new.get('web', 'ru')[2][0]*10}.0" }
    it { expect(@deals_demo[1]["data"]['deals'][0]['status']).to eq "open" }
