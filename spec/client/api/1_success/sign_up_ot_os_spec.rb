...
RSpec.describe "Registration ot and os params success " do
  before(:all) do
     $registration = Reg.post_sign_up_offer[0]
  end
  ...
  context "params" do
    ...
    it { expect($registration['data']['currency']).to eq 'RUB' }
    ...
    it { expect($registration['data']['first_account']).to eq nil }
