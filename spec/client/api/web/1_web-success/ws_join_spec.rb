...
RSpec.describe "join session" do
  before(:context) do
    ...
    #  sleep (Time.parse(@deals_won['finished_at']) - Time.parse(@deals_won['created_at'])).to_i + 5
     ...
     sleep 1
     null = "null"
     @msg_ws_deal = $msg_deals.to_h

  end

  context "param create deal" do
    it { puts "#{@msg_ws_deal}".yellow }
    it { puts "#{@msg_ws_deal['payload']}".yellow }
    # it { expect(@msg_ws_deal['topic']).to eq "base" }

    it { expect($msg_deals[:payload][:trend]).to eq "call" }
    ...
    # it { expect(@msg_ws_deal['payload']['ric'].to_f).to eq "GOL/OTC" }
    # it { expect(@msg_ws_deal['payload']['bet']).to eq "#{@amount.to_f}" }
