...
RSpec.describe "Deals real success, turbo validations put(won)" do
  before(:all) do
    @deal_list_data = DealsApp.new.list_close_demo("ios", "ru", $uuid, "1.0", Cookies.where(stage: "#{ENV['stage']}").last.authtoken)[0]['data']['deals'][0]
    ...
    @closed_deal_data = DealsApp.new.list_close_demo("ios", "ru", $uuid, "1.0", Cookies.where(stage: "#{ENV['stage']}").last.authtoken)[0]['data']['deals'][0]
    sleep 1
  end
  ...
  context "params" do
    ...
    it {expect(@closed_deal_data['win']).to eq 1000 + Assets.new.get('ios', 'ru')[1][0]*10 }
...
RSpec.describe "Deals real success, turbo validations call(lost)" do
  before(:all) do
    @deal_list_data = DealsApp.new.list_close_demo("ios", "ru", $uuid, "1.0", Cookies.where(stage: "#{ENV['stage']}").last.authtoken)[0]['data']['deals'][1]
    @closed_deal_data = DealsApp.new.list_close_demo("ios", "ru", $uuid, "1.0", Cookies.where(stage: "#{ENV['stage']}").last.authtoken)[0]['data']['deals'][1]
    sleep 1
  end
