...
RSpec.describe "Deals success turbo" do
  before(:context) do
    ...
    @deals_demo = DealsApp.new.demo_create(
      true, "1000", "GOL/OTC", "turbo", "call", "mouse", Time.now.to_i,
      @expired, 'ru', 'ios', $uuid, "1.0",
  ...
  context "params" do
    ...
    it { expect(@deals_demo[1]["data"]['deals'][0]['asset']).to eq Assets.new.get('ios', 'ru')[3][0] }
    it { expect(@deals_demo[1]["data"]['deals'][0]['ric']).to eq Assets.new.get('ios', 'ru')[3][0] }
    ...
    it { expect(@deals_demo[1]["data"]['deals'][0]['payment']).to eq "#{1000 + Assets.new.get('ios', 'ru')[1][0]*10}.0".to_f }
    it { expect(@deals_demo[1]["data"]['deals'][0]['status']).to eq "open" }
    it { expect(@deals_demo[1]["data"]['deals'][0]['option_type']).to eq "turbo"  }
    it { expect(@deals_demo[1]["data"]['deals'][0]['payment_rate']).to eq Assets.new.get('ios', 'ru')[1][0] }
...
RSpec.describe "Deals success turbo" do
  before(:context) do
    ...
    @deals_demo = DealsApp.new.demo_create(
          true, "1000", "GOL/OTC", "binary", "call", "mouse",
          Time.now.to_i, @expired, 'ru', 'ios', $uuid, "1.0",
          Cookies.where(stage: "#{ENV['stage']}").last.authtoken)
  ...
  context "params" do
    ...
    it { expect(@deals_demo[1]["data"]['deals'][0]['entrie_rate']).to be_a(Float)  }
    it { expect(@deals_demo[1]["data"]['deals'][0]['bet']).to eq 1000  }
    it { expect(@deals_demo[1]["data"]['deals'][0]['payment']).to eq "#{1000 + Assets.new.get('ios', 'ru')[2][0]*10}.0".to_f }
    it { expect(@deals_demo[1]["data"]['deals'][0]['status']).to eq "open" }
    it { expect(@deals_demo[1]["data"]['deals'][0]['option_type']).to eq "binary"  }
    it { expect(@deals_demo[1]["data"]['deals'][0]['payment_rate']).to eq Assets.new.get('ios', 'ru')[2][0] }
    it { expect(@deals_demo[1]["data"]['deals'][0]['demo']).to eq true }
    it { expect(@deals_demo[1]["data"]['deals'][0]['end_rate']).to eq 0.0 }
    it { expect(@deals_demo[1]["data"]['deals'][0]['win']).to eq 0 }
    it { expect(@deals_demo[1]["data"]['errors']).to eq nil }
