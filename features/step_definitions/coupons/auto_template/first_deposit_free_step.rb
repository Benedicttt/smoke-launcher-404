Given(/^Template First Deposit Free Deals$/) do
  DRIVER.get CommonSetting[:url_page_crm] + "/coupons/welcome/edit?handler=first_deposit_free_deals"
  sleep 1

  DRIVER.find_element(:id, "coupon_name").clear
  DRIVER.find_element(:id, "coupon_name").send_keys("Template First Deposit Free Deals Change to #{Time.now}")
